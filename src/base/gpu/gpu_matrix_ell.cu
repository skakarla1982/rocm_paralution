...
bool GPUAcceleratorMatrixELL<ValueType>::ConvertFrom(const BaseMatrix<ValueType> &mat) {
  ...
  if ((cast_mat_csr = dynamic_cast<const GPUAcceleratorMatrixCSR<ValueType>*> (&mat)) != NULL) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_ell_max_row<int, int, 256>), dim3(GridSize), dim3(BlockSize), 0, 0, nrow, cast_mat_csr->mat_.row_offset,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_ell_csr_to_ell<ValueType, int>), dim3(GridSize2), dim3(BlockSize2), 0, 0, nrow, max_row, cast_mat_csr->mat_.row_offset,
